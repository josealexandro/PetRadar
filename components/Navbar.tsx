...
export function Navbar() {
  ...
  return (
    <header className="header-top-safe border-b border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 pb-4 pt-3 sm:px-6 sm:pb-5 sm:pt-4">
        <Link
          href="/"
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {loading ? (
            ...
            <>
              {/* Desktop: conta + avatar + sair visíveis */}
              <div className="hidden min-w-0 items-center gap-2 sm:gap-3 md:flex">
                <span className="text-sm text-zinc-600 dark:text-zinc-400 sm:inline">
                  <span className="font-medium tabular-nums text-zinc-900 dark:text-zinc-100">
                    {animalsHelpedCount}
                  </span>{" "}
                  {animalsHelpedCount === 1 ? "animal ajudado" : "animais ajudados"}
                </span>
                <span className="hidden max-w-[120px] truncate text-sm text-zinc-700 dark:text-zinc-300 lg:inline">
                ...
                {user.photoURL ? (
                  ...
                  <span
                    className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-zinc-600 text-sm font-medium text-white"
                    aria-hidden
                  >
              ...
              {/* Mobile: só avatar (menu com Criar conta + Sair) para não esconder a foto */}
              <div className="relative flex items-center gap-2 md:hidden" ref={menuRef}>
                <button
                  type="button"
                  onClick={() => setMenuOpen((o) => !o)}
                  className="flex h-9 w-9 shrink-0 overflow-hidden rounded-full border-2 border-transparent focus:border-zinc-400 focus:outline-none"
                ...
                >
                  {user.photoURL ? (
                    ...
                    <span className="flex h-full w-full items-center justify-center rounded-full bg-zinc-600 text-sm font-medium text-white">
                      {user.displayName?.charAt(0) ?? user.email?.charAt(0) ?? "?"}
                    </span>
                  )}
                </button>
                  ...
                  <div className="absolute right-0 top-full z-20 mt-1 w-48 rounded-xl border border-zinc-200 bg-white py-1 shadow-lg dark:border-zinc-700 dark:bg-zinc-900">
                    {user.isAnonymous && (
                      <Link
                        ...
                        className="block px-4 py-2.5 text-sm font-medium text-emerald-600 dark:text-emerald-400"
                      >
                        Criar conta
                      </Link>
                    )}
                    <button
                      ...
                      className="w-full px-4 py-2.5 text-left text-sm text-zinc-700 dark:text-zinc-300"
                    >
                      Sair
                    </button>
                  </div>
